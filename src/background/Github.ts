...
import { GithubPullRequest, GithubPullRequestChanges, GithubReview } from '@html/types/Github';
...
class Github {
	...
	public fetchPullRequests = async () => {
		const username = extensionUser.getData().login;
		...
		if (message.type === 'get_pull_requests') {
			...
			if (cachedPr) {
				...
				extensionMessageHandler.sendMessageToAll({
					...
					data: this.__getPullRequestsChangesObject()
				});
			}
	...
	private __getUpdatedReviews = (pr: GithubPullRequest, viewedPullRequest?: GithubPullRequest) => {
		...
		return filter(
			values(pr.reviews),
			(review: GithubReview) =>
				review.state !== 'PENDING' && review.date !== viewedPullRequest.reviews[review.user]?.date
		);
	};
	...
	private __getPullRequestsChangesObject = (): Record<string, GithubPullRequestChanges> => {
		return transform(
			this.__pullRequests,
			(result, pr) => {
				result[this.__getCachePullRequestKey(pr)] = this.__getPullRequestChanges(pr);
			},
			{}
		);
	};
	...
	private __updatePullRequestChanges = (pullRequest: GithubPullRequest): GithubPullRequestChanges => {
		const previousPRStatus: GithubPullRequestChanges = {
			comments: 0,
			commits: 0,
			reviews: 0
		};
		...
		if (cachedPRData && cachedPRData.viewedPullRequest) {
			previousPRStatus.comments =
				cachedPRData.viewedPullRequest.comments + cachedPRData.viewedPullRequest.reviewComments;
	...
	private __getPullRequest = async (userPullRequest: GithubPullRequest) => {
		...
		if (cachedPRData) {
			if (moment(cachedPRData.pullRequest.updated_at) >= moment(userPullRequest.updated_at))
				return cachedPRData.pullRequest;
		}

		// If updated, fetch it again and update updated values
		const { owner: user, repository, number } = userPullRequest;
		const pullRequest = await GithubApi.getPullRequestInfo({
			data: { user, repository, number }
		});
	...
	private __sendPullRequestsLoading = () => extensionMessageHandler.sendMessageToAll({ type: 'pull_requests_loading' });
	private __sendPullRequests = () =>
		extensionMessageHandler.sendMessageToAll({
