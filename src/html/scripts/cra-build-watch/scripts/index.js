...
const config =
	concatenatedVersion >= 212
		? (isEjected ? importCwd('./config/webpack.config') : importCwd('react-scripts/config/webpack.config'))(
				'development'
		  )
		: isEjected
...
const getClientEnvironment = isEjected ? importCwd('./config/env') : importCwd('react-scripts/config/env');
...
config.output.chunkFilename = chunkNameToUse.slice(-3) !== '.js' ? `${chunkNameToUse}.js` : chunkNameToUse;
