...
exports.getReactScriptsVersion = function getReactScriptsVersion(cliVersion) {
	if (cliVersion) {
		const versions = {
			...
			concatenatedVersion: Number(`${semver.major(cliVersion)}${semver.minor(cliVersion)}${semver.patch(cliVersion)}`)
		};
		return versions;
	}
	...
	const versions = {
		...
		concatenatedVersion: Number(`${semver.major(version)}${semver.minor(version)}${semver.patch(version)}`)
	};
	return versions;
};
