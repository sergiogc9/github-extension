...
const ExtensionPopupPullRequests: React.FC<ComponentProps> = props => {
	...
	const getPullRequestChangesContent = React.useCallback((pullRequest: GithubPullRequest) => {
		...
		return (
			<div className='pr-changes-content'>
				{prChanges && (
					<>
						{!!prChanges.commits && <div title='New commits'><FontAwesomeIcon name="code-commit" type="duo" /><span>{prChanges.commits}</span></div>}
						{!!prChanges.comments && <div title='New comments'><SymbolicIcon name="chat-conversation-alt" type="solid" /><span>{prChanges.comments}</span></div>}
						{!!prChanges.reviews && <div title='Reviews updated'><SymbolicIcon name="clipboard-check" type="solid" /><span>{prChanges.reviews}</span></div>}
					</>
				)}
	...
	const getPullRequestStatusContent = React.useCallback((pullRequest: GithubPullRequest) => {
		...
		if (pullRequest.checks.pending > 0) return <div className='pending-circle'></div>;
